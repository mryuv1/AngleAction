function [track, numOfRoundHit, numOfHorizontalHit, numOfVerticalHit] = calc_track_until_px_is_zero(initial_conds, w, isSecondHit)
    ...
    if (~isSecondHit)
            [numOfHorizontalHit, numOfRoundHit, numOfVerticalHit] = ...
                                checkIfOnBorderline(initial_conds);
    else
    ...
    tsapn = [0 50];
    opt = odeset('AbsTol',5e-11,'RelTol',5e-11, 'Events',@EventsFunction);
    ...
    while abs(final_position(3))>10e-13
        slope = get_step_slope(final_position);
        ...
        [final_position, hitOnCircle, horizontalHit, verticalHit] = correct_coords(final_position);
        numOfRoundHit = numOfRoundHit + hitOnCircle;
        numOfHorizontalHit = numOfHorizontalHit + horizontalHit;
        numOfVerticalHit = numOfVerticalHit + verticalHit;
